...
class WeatherDataParser:
    """
    Utility class for parsing weather JSON data.
    """
    
    @staticmethod
    def parse_weather_json(json_source: str) -> List[MEASUREMENT]:
        """
        Parse JSON weather data and convert to MEASUREMENT objects.
        ...
        try:
            ...
            print(f"Successfully loaded {len(measurements)} measurements from JSON")
            return measurements
...
class ModelDataset(DATASET):
    ...
    """
    def __init__(self, measurements: List[MEASUREMENT] = None, model_source: str = "", auto_load: bool = True):
...
class CastDataset(DATASET):
    ...
    def __init__(self, measurements: List[MEASUREMENT] = None, station_id: str = "", 
                 json_source: str = "", noise_level: float = 0.1, auto_load: bool = False):
    ...
    def load_data(self) -> List[MEASUREMENT]:
        ...
        If json_source is provided, parse it and add realistic noise.
        """
        if self.json_source:
            # Lade Basis-Daten mit dem gemeinsamen Parser
            ...
            print(f"Generated {len(cast_measurements)} cast measurements with noise level {self.noise_level}")
